...
catalog: true
header-style: text
header-img: "img/posts/2026-06-29-mixed-topics/cover.png"
tags:
---
...
*图：同一模型在不同Harness下成功率最大相差86%，而Harness选择几乎不增加Token成本。*
...
模型只做一件事：输入文本，预测输出文本。Claude、GPT、Gemini、DeepSeek，再好再贵的模型，都只能预测下一个 token。

Harness（线束/执行框架）负责的是"让文本变成动作"这件事。它包括：
把任务和可用工具格式化成 prompt、解析模型输出里的动作指令、执行这些动作（调 API、读文件、跑代码）、把结果喂回给模型、管理执行循环、处理重试和错误、决定什么时候停下来。
...
- **claude_code 是专门为 Claude 设计的。** Garness 的适配深度可能比 garness 本身的"好坏"更重要。
...
- How to actually evaluate AI agents at scale — Hugging Face，2026-06-24。https://huggingface.co/blog/darubberduckiee/using-braintrust-to-eval-agentic-setups
- Exgentic/agent-llm-traces - Multi-Benchmark LLM Agent Traces — Exgentic，约 2026-05-07。https://huggingface.co/datasets/Exgentic/agent-llm-traces
- Agent Harness：2026年AI工程的核心范式 — 腾讯云开发者社区，约 2026-04。https://cloud.tencent.com/developer/article/2698416
- Agent Harness for Large Language Model Agents: A Survey — GloriaaaM（预印本作者），2026-04-09（v3）。https://huggingface.co/datasets/GloriaaaM/LLM-Agent-Harness-Survey
