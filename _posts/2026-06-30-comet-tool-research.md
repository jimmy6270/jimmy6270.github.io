...
如果你经常让 AI agent 改一个跨文件、跨步骤的功能，最烦的往往不是"它会不会写代码"，而是它做到一半忘了需求、跳过测试、文档不同步，或者换个会话就接不上。[Comet](https://github.com/rpamis/comet) 这个项目想解决的正是这类长任务失控问题。

先说结论：Comet（[GitHub](https://github.com/rpamis/comet)｜[npm](https://www.npmjs.com/package/@rpamis/comet)）不是单纯的代码生成工具。它更像一个 agent skill harness,把 OpenSpec、Superpowers、CodeGraph 和一组项目脚本串起来,让 AI 编码任务按 Open、Design、Build、Verify、Archive 五个阶段推进。这个判断来自项目 README、中文 README、package.json、Auto Transition 文档和 CHANGELOG。它的使用价值,也要放在"阶段化工作流"里看,而不是拿它和一个普通聊天窗口对比。
